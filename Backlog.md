...
- **Phase 4 — Core App: 🟢 NEARLY COMPLETE — all routes are real screens.** DONE: role `AppLayout` (sidebar+drawer); all 3 dashboards; family + nanny calendars w/ calendar-first booking flow (status routing confirmed/pending/same-day); shared bookings pages (family cancel, nanny accept/decline); nanny directory + full profile (shared, booking buttons family-only); family + nanny My Profile editors (nanny completeness indicator, verified badges read-only); family billing; shared policies; full messaging (family/nanny/admin, All/Families/Nannies filter, Lucy/David reply tagging); admin nannies + families management (tabs, approve/reject, stage advance); admin platform bookings (filters, cancel); admin billing (4 tabs + donation tracker); admin analytics (5 tabs, live metrics); admin settings (6 config tabs). All hooks: `useBookings`, `useAdmin`, `useNannies`, `useMessages`, `useProfile`. Indexes added for every compound query.
  - **REMAINING Phase 4 polish (deferred, lower priority):** (1) Public marketing homepage + family/nanny info pages + application forms at `/` for signed-out visitors (currently `/` redirects to login; signup collects minimal fields — see D7b). (2) Reviews submission form/modal (prompt cards + admin-only storage rules exist; the write-a-review form UI is stubbed via dashboard buttons). (3) Referral link display surface (code + `referralUrl` exist on every user; needs a "share your link" UI panel). (4) Drag-to-select calendar slot (currently click-a-day). (5) Recurring booking 48h auto-cancel + same-day/open-booking email triggers — need a Cloud Function / scheduled job (email provider is a BLOCKING open item). (6) Admin "Create Booking" form (override). (7) Excel/PDF export buttons are present but not wired to a generator.
- **Phase 5 — Security Audit: ✅ COMPLETE & COMMITTED.** Worked all 17 checklist sections → `docs/security-audit.md`. Fixes: booking list authz tightened to per-doc `read` (closed cross-tenant read); `users` kept non-listable + nanny name denormalized; verified-badge immutability; firebase 10→12 (vulns 14→4, remainder dev-only); App Check (reCAPTCHA v3); tight CSP; `access.ts` + tests mirror the booking rule. 21 tests green. **Top launch follow-ups:** emulator rules unit tests (Firebase CLI was unavailable), Firestore backups/PITR, in-app MFA, privacy policy content, monitoring alerts, clear dev-only npm advisories via vite 8.
- (was) **Phase 5 — Security Audit: NEXT**
